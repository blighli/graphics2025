...
double GLFWWrapper::scrollYOffset = 0.0;
...
bool GLFWWrapper::initGlfw(int width, int height, const char *title)
{
    ...
    glfwSetScrollCallback(window, GLFWWrapper::scrollCallback);
...
double GLFWWrapper::consumeScrollYOffset()
{
    if (!initialized)
    {
        spdlog::warn("GLFW is not initialized.");
    ...
    const double offset = scrollYOffset;
    scrollYOffset = 0.0;
    return offset;
}
