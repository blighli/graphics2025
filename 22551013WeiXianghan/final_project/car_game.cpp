...
#include <car_game/car.h>
#include <car_game/fixed_camera.h>
...
int main()
{
    ...
    // 打印当前的工作目录（debug用）
    std::cout << "Current Working Directory: " << FileSystem::getPath("") << std::endl;
    // 窗口初始化
    ...
    // 为所有物体添加光照和阴影的shader
    Shader shader("resources/shaders/light_and_shadow.vs", "resources/shaders/light_and_shadow.fs");
    ...
    // 汽车模型
    Model carModel("resources/objects/Lamborghini/Lamborghini.obj");
    // 相机模型
    Model cameraModel("resources/objects/camera-cube/camera-cube.obj");
    // 赛道模型
    Model raceTrackModel("resources/objects/race-track/race-track.obj");
    // STOP牌模型
    Model stopSignModel("resources/objects/StopSign/StopSign.obj");
    // ---------------------------------
    // shader 纹理配置
    // ---------------------------------
    ...
    printf("Initialization Finished! Starting Render Loop...\n");
...
GLFWwindow* windowInit()
{
    ...
    // 创建窗口
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, u8"car game", NULL, NULL);
    if (window == NULL) {
