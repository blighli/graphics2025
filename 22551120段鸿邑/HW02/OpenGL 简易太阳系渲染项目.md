...
- 纹理路径必须正确：若纹理加载失败，程序会打印错误信息，需优先检查路径是否为绝对路径或相对路径正确；

- 依赖库版本适配：确保 GLFW、GLM 版本兼容，避免因版本差异导致的函数接口错误；

- 深度测试开启：项目已启用 `GL_DEPTH_TEST` 并设置 `GL_LEQUAL`，确保 3D 物体的遮挡关系正确；

- 鼠标交互范围：仅左键点击有效，点击窗口空白区域会提示“未点击到任何球体”；

- 窗口大小调整：调整窗口后，投影矩阵会自动更新，渲染效果保持正常，无需重启程序。

# 扩展方向

- 增加更多天体：如月球（绕地球旋转）、火星、金星等，复用球体生成与纹理加载逻辑；

- 优化光照效果：添加点光源移动、聚光灯效果，或实现 PBR（基于物理的渲染）；

- 增强交互：支持鼠标拖拽旋转相机、滚轮缩放视角，提升观察体验；

- 添加纹理动画：如太阳耀斑动画、地球云层流动效果；

- 优化性能：使用实例化渲染（Instanced Rendering）批量绘制多个相似天体，减少绘制调用。
> （注：文档部分内容可能由 AI 生成）